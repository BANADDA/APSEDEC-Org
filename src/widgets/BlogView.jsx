...
const BlogView = ({ blog }) => {
    ...
    const userImageUrl = blog.userImageUrl || 'default-avatar.png'; // Fallback to a default avatar if not available
            ...
            {blog.galleryImages && blog.galleryImages.length > 0 && blog.galleryImages.map((imageUrl, index) => (
                <Box key={index} sx={{ mt: 4, textAlign: 'center' }}>
                    ...
                    {blog.projectDetails && blog.projectDetails[index] && (
                        <Typography variant="body2" sx={{ mt: 2, textAlign: 'left', ml: 2 }}>
